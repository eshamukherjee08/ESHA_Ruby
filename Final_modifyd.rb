class Dictionary
  ...
  def initialize
    @@option_list = Array.new              # to store the option's list read from words.txt
    @@avail_list = Array.new               # to store match for each individual option wrt to wrong word.
  end
  ...
  def compare                                 #function to find longest match(s)
   ...
   for i in 0...fk.size
     w_match << mat[i] if(maximum == fk[i].length)
   end
